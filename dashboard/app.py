...
filtered = filtered[filtered["price_chf_month"].fillna(10**9) <= max_price]
...
st.dataframe(
    filtered[[
        "site", "title", "price_chf_month", "surface_m2", "district",
...
for _, row in filtered.iterrows():
    ...
    if district in DISTRICT_COORDS:
        ...
        popup = f"{row['title']}<br>{row.get('price_chf_month')} CHF<br>{row.get('surface_m2')} m²<br>Score {row.get('score')}"
