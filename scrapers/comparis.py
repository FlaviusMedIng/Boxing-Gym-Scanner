...
class ComparisScraper(BaseScraper):
    ...
    def parse_list_page(self, soup, base_url: str) -> list[dict]:
        listings: list[dict] = []
        cards = soup.select("article") or soup.select("a[href*='/immobilien/']")
        for card in cards:
            link = card.find("a", href=True) if hasattr(card, "find") else None
            if link is None and getattr(card, "name", "") == "a":
                link = card
            if not link:
                continue
            ...
            if not any(term in blob.lower() for term in ["gewerbe", "commercial", "arcade", "bureau", "atelier"]):
                continue
